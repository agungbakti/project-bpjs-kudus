...
<body>
    ...
    <script>
        // Script untuk real-time update visual DAN AJAX Save
    ...
    function saveData(id, field, value) {
        $.ajax({
            url: 'update_data_ajax.php', // Path ke file PHP yang baru dibuat
            type: 'POST',
            data: {
                id: id,
                field: field,
            ...
            error: function(xhr, status, error) {
                ...
                console.error('Gagal menyimpan data:', response.message);
                // console.log(xhr.responseText); // Tampilkan detail error dari server
                //  Swal.fire({
                //     icon: 'error',
                //     title: 'Error!',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        statusSelects.forEach(select => {
            select.addEventListener('change', function() {
                ...
                updateRowStatus(row); // Update visual
                saveData(id, 'status', value); // Save to database
            });
        });
        
        // Add event listeners for keterangan select
        keteranganSelects.forEach(select => {
            select.addEventListener('change', function() {
                ...
                const value = this.value;
                updateRowStatus(row); // Update visual
                saveData(id, 'keterangan', value); // Save to database
            });
        });
        
        // Add event listeners for nama input
        namaInputs.forEach(input => {
            input.addEventListener('blur', function() { // Gunakan 'blur' agar tidak terlalu sering mengirim request saat mengetik
                const row = this.closest('tr');
                const id = this.dataset.id;
                const value = this.value.trim();
                updateRowStatus(row); // Update visual
                saveData(id, 'nama', value); // Save to database
            });

            // Optional: juga update visual saat mengetik, tapi save hanya saat blur
    ...
    // Nonaktifkan atau sesuaikan fungsi loadData interval jika sudah menggunakan AJAX
    // Jika AJAX sudah berhasil, tidak perlu refresh data setiap 5 detik
